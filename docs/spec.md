...
| **2.5 — Web interface** ✅ prototyped | Visual UI (not a chat, 100% voice) so it doesn't depend on the terminal: a React SPA (`src/frontend/`, built by Vite into `src/static_dist/`, served by `api.py`) split across two client-side routes: `/` (`Picker.tsx`) picks/creates a recipe (microphone button, visual status, tap-a-card list of local recipes); `/cook` (`Cook.tsx`) is the hands-free cooking screen once a recipe is active — a small animated face (`components/Face/`: idle/listening/thinking/speaking/happy/confused expressions, wandering eyes, blinking) with "◀ Anterior"/"Siguiente ▶" pinned to the screen edges, laid out for a phone propped up in landscape on the counter; tapping the face asks a follow-up question the same way the old mic button did. FastAPI backend (`src/api.py`) exposes `POST /api/question` (uploads the audio recorded in the browser, transcribes it locally with whisper, routes it through `recipe_engine.py`, returns the spoken answer as audio for the caller's own device to play) and `GET /api/state`. The browser only records audio (`MediaRecorder`) and uploads it — it never transcribes or recognizes speech on its own, so as not to break the "100% local" principle from §2. | Phase 2 |
...
| **5 — Dedicated hardware (optional)** | Port to a Raspberry Pi 5 or another mini PC for "always on" use in the kitchen, like the original BMO project | Phases 1-3 stable on the PC |
