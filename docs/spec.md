...
| **2.5 — Web interface** ✅ prototyped | Visual UI (not a chat, 100% voice) so it doesn't depend on the terminal, split across two routes: `/` (`src/static/index.html`) picks/creates a recipe (microphone button, visual status, tap-a-card list of local recipes); `/cook` (`src/static/cook.html`) is the hands-free cooking screen once a recipe is active — a small animated face (idle/listening/thinking/speaking/happy/confused expressions, wandering eyes, blinking) with "◀ Anterior"/"Siguiente ▶" pinned to the screen edges, laid out for a phone propped up in landscape on the counter; tapping the face asks a follow-up question the same way the old mic button did. FastAPI backend (`src/api.py`) exposes `POST /api/question` (uploads the audio recorded in the browser, transcribes it locally with whisper, routes it through `recipe_engine.py`, returns the spoken answer as audio for the caller's own device to play) and `GET /api/state`. Plain HTML/CSS/JS throughout, no frameworks or CDN, orange/cream kitchen styling. The browser only records audio (`MediaRecorder`) and uploads it — it never transcribes or recognizes speech on its own, so as not to break the "100% local" principle from §2. | Phase 2 |
